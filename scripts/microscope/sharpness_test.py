...
def run_pass(arm: Arm, scope: Microscope, a1: Sequence[float], z_values: list[float],
            ...
            masked: bool, ordinal: int,
            stop_after_falls: int | None = None,
            ) -> tuple[list[dict[str, Any]], list[FocusSample], bool]:
    """Step through ``z_values``, scoring one frame each.

    ``stop_after_falls`` ends the pass once that many CONSECUTIVE samples have
    scored below their predecessor -- climbing is never interrupted, and a
    single noisy dip resets the counter. ``None`` disables it.

    Returns ``(samples, focus_samples, truncated)``. ``truncated`` is True only
    when the early stop fired, and it matters: prominence is peak/median, so a
    verdict computed on a partial scan is not the same measurement as one
    computed on a whole one.
    """
    ...
    for i, z in enumerate(z_values, start=1):
        ...
        if rec.get("usable"):
            print("        mean=%7.2f  score=%.6g  %s" % (rec["mean"], rec["score"], dest.name),
                 flush=True)
            if focus_samples and stop_after_falls:
                # Compare against the previous USABLE sample, not the previous
                # z step: an unusable frame raises above, so they cannot differ
                # here, but the intent is "the last thing we actually scored".
                falls = falls + 1 if rec["score"] < focus_samples[-1].score else 0
            focus_samples.append(sample)
            if stop_after_falls and falls >= stop_after_falls:
                truncated = True
                print("        [early-stop] %d consecutive falls from the running "
                      "peak; the rest of this pass would not change the answer. "
                      "Scan marked TRUNCATED." % falls, flush=True)
                break
        else:
            raise SweepError("%s: %s -- stopping rather than climbing blind"
                             % (label, rec.get("reason")))
    return samples, focus_samples, truncated
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--stop-after-falls", type=int, default=3,
                    help="end the COARSE pass after this many CONSECUTIVE samples "
                         "score below their predecessor (default 3). Climbing is "
                         "never interrupted and one noisy dip resets the count. "
                         "Replayed on the 2026-08-11 A1 curve: 1 fires at sample 2 "
                         "on noise, 3 fires at sample 29 of 35, genuinely past the "
                         "maximum.")
    ap.add_argument("--no-early-stop", action="store_true",
                    help="sample every planned coarse point. Use when the scan will "
                         "be judged on prominence and you want the median computed "
                         "over the full range -- truncation changes it.")
    ...
    ap.add_argument("--min-prominence", type=float, default=1.5,
                    help="af.judge_scan: reject a fine scan whose peak/median ratio is below "
                         "this (default 1.5; a dot-free sweep on this rig measured ~1.16, a "
                         "real peak ~50)")
    ap.add_argument("--min-absolute-score", type=float, default=0.0,
                    help="af.judge_scan: reject a fine scan whose best score is below this, "
                         "even if the prominence looks fine (default 0.0 -- off; prominence "
                         "alone is NOT sufficient, a single dust speck can spike it on an "
                         "otherwise flat scan, so set this from a known-good peak score on "
                         "this rig/method/illumination before relying on it)")
    ap.add_argument("--max-unusable-frac", type=float, default=1.0,
                    help="af.judge_scan: reject a fine scan where more than this fraction of "
                         "samples were unusable (default 1.0 -- off)")
    return ap
...
def run(args: argparse.Namespace) -> int:
    ...
    finite("--min-prominence", args.min_prominence)
    finite("--min-absolute-score", args.min_absolute_score)
    finite("--max-unusable-frac", args.max_unusable_frac)
    if args.min_prominence < 0:
        raise SystemExit("--min-prominence must be >= 0")
    if args.min_absolute_score < 0:
        raise SystemExit("--min-absolute-score must be >= 0")
    ...
    try:
        ...
        coarse_samples, coarse_focus, coarse_truncated = run_pass(
            arm, scope, a1, plan["coarse_z"], out_dir, "coarse", args.settle,
            args.method, args.min_mean, masked, args.frame_ordinal,
            stop_after_falls=(None if args.no_early_stop else args.stop_after_falls))
        samples += coarse_samples

        if args.execute:
            if coarse_truncated:
                print("[pass 1/2] TRUNCATED by the early stop: %d of %d coarse points "
                      "were taken. Prominence is peak/median, so this verdict is not "
                      "the same measurement the full range would have given -- re-run "
                      "with --no-early-stop before trusting a marginal PASS."
                      % (len(coarse_focus), plan["n_coarse"]))
            ...
            fine_samples, fine_focus, _ = run_pass(
            ...
            # judge_scan applies the operator's own thresholds -- see the CLI
            # help for each -- and reports which one(s) failed, so a dot-free
            # sweep says "prominence 1.16, below the floor" instead of only
            # aborting on saturation or a boundary peak.
            judged = af.judge_scan(
                fine_report, min_prominence=args.min_prominence,
                min_absolute_score=args.min_absolute_score,
                max_unusable_frac=args.max_unusable_frac, require_interior=False)
            print("[result] judge_scan: %s (peak/median=%s, best=%s, unusable_frac=%s)"
                  % ("usable" if judged["usable"] else "NOT usable",
                     fine_report["peak_over_median"], fine_report["best_score"],
                     fine_report["unusable_frac"]))
            if not judged["usable"]:
                raise SweepError("no focal plane here: %s" % "; ".join(judged["reasons"]))
        ...
        try:
            ...
            (out_dir / "manifest.json").write_text(json.dumps({
                ...
                "judge_scan": judged,
                "judge_scan_thresholds": {"min_prominence": args.min_prominence,
                                          "min_absolute_score": args.min_absolute_score,
